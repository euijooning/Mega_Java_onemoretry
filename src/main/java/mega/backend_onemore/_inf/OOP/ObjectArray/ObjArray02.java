...
public class ObjArray02 {
  public static void main(String[] args) {
    ...
    // [1] : 객체 생성
    Person02[] pa;				// Person 타입의 객체 배열 변수 pa 선언.
    pa = new Person02[5];		// Person 객체의 참조값을 원소로 가지는 배열 공간을 5개 생성 --> 5명 객체 생성할테니 주소 저장해라.

    // [2] : 반복문(for)을 돌면서 Person() 객체 생성
    for( int i=0; i < pa.length; i++ ) {
      pa[i] = new Person02( i+"번 후보자", i+20 );  // i만 입력하면 안되나 문자열을 붙이면 문자열로 전달.
