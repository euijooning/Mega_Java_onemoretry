package mega.backend_onemore._inf.OOP.ParseInt;
...
// [ ! ] : 왜 변환이 필요한가?
// 1+2 숫자끼리의 연산은 3이지만,
// 문자열끼리의 연산은 12가 되기 때문에 숫자로 변환 후 연산을 수행해야 한다.

// 문자열을 숫자로 변환시키는 방법 --> Integer.parseInt() 사용.
// parseInt() --> Integer 클래스의 static으로 지정
// --> 따라서, 객체의 생성없이 바로 "클래스명.parseInt()"로 직접 사용이 가능.
// 두 번째 인자 값으로 진수를 지정 --> 해당 진수에 맞는 값이 됨
// ==> Integer.parseOmt("1000", ______);
// ex. 1000을 2진수로 출력 Integer.parseOmt("1000", 2);
